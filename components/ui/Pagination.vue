  ...
  methods: {
    ...
    previousPageGroup() {
      if (this.currentPage !== 1) {
        const pageToSet = this.currentPage - 1;

        if (
          pageToSet > this.navSideBtnsQuantity &&
          pageToSet < this.listLength - this.navSideBtnsQuantity
        ) {
          this.startIndex--;
        }
        this.$emit('change', this.currentPage - 1);
  ...
  computed: {
    ...
    listLength() {
      if (this.itemsPerPage > 0) {
        return Math.ceil(this.recordsCount / this.itemsPerPage);
      } else {
